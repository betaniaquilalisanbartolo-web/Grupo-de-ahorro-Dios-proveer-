  ...
  if df_socios.empty:
    ...
    with tab1:
      ...
      with st.form("form_ahorro", clear_on_submit=True):
        ...
        if btn_ahorro:
          if monto_ahorro <= 0:
            st.error("El monto ahorrado debe ser mayor a cero.")
          else:
...
# ==========================================
  ...
  if df_socios.empty:
      ...
      if st.button("Aprobar y Registrar Préstamo"):
        ...
        with motor.begin() as conn:
          conn.execute(
              ...
              {
                  "socio_id": socio_id,
                  "monto_prestado": float(monto_solicitado),
    ...
    with tab3:
      ...
      if df_rep_p.empty:
        st.info(
            "No se registraron préstamos vigentes o emitidos en el mes"
            f" {mes_rep}/{anio_rep}."
        )
      else:
            ...
            with motor.begin() as conn:
              conn.execute(
                  text("""
                    ...
                    """),
                  {
                      "monto": float(e_monto_p),
                      "tasa": float(e_tasa_p),
                      "plazo": int(e_plazo_p),
                      "int_tot": float(e_int_tot),
                      "monto_tot": float(e_monto_tot),
                      "fecha": str(e_fecha_p),
                      "estado": e_estado_p,
                      "anio": e_fecha_p.year,
                      "id": id_p_sel,
          ...
          with motor.begin() as conn:
            res_p = conn.execute(
                    ...
                    """),
                {
                    ...
                    "capital": float(m_capital),
                    "interes": float(m_interes),
            ...
            ) or (tot_pagado_total >= float(datos_p["monto_total"]))
  ...
  if gran_total_ahorrado == 0:
    ...
    query_ahorros_mes = """
            ...
            WHERE s.estado = 'Activo' 
              AND EXTRACT(YEAR FROM a.fecha) = :a 
              AND EXTRACT(MONTH FROM a.fecha) <= :m
    ...
    st.success(
        ...
        " que dicho capital permaneció en la caja social (Ponderación Mes"
        " a Mes)."
    )
