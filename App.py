...
# ==========================================
    ...
    with engine.connect() as conn:
        df_socios = pd.read_sql("SELECT id, nombre FROM socios WHERE estado = 'Activo'", conn)
...
# ==========================================
    ...
    if df_socios.empty:
        ...
        * **Cuota Mensual Estimada:** C$ {cuota_mensual:,.2f} / mes
        ...
        query_p = """
            SELECT p.id as "ID", s.nombre as "Socio", p.monto_prestado as "Monto Prestado (C$)", p.interes_total as "Interés Total (C$)", p.monto_total as "Total a Pagar (C$)", p.fecha_inicio as "Fecha", p.estado as "Estado" 
            FROM prestamos p 
            JOIN socios s ON p.socio_id = s.id 
            ORDER BY p.id DESC
        """
...
# ==========================================
    ...
    with tab2:
        ...
        st.warning("Al ejecutar el Cierre Anual, se registrará el resumen de este periodo en el historial. Los balances de ahorro se archivarán para empezar un nuevo ciclo el próximo año manteniendo la lista de socios.")
        ...
        if st.button("🚀 Ejecutar Cierre y Reiniciar Año"):
            ...
            with engine.begin() as conn:
                ...
                # Archivar limpiando tablas de la caja activa
