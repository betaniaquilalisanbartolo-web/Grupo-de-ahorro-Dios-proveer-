...
# ==========================================
st.set_page_config(
    page_title="Caja de Ahorro Comunitario", page_icon="ðŸ’°", layout="wide"
)
...
if opcion == "ðŸ“Š Panel General":
    ...
    with motor.connect() as conn:
        ...
        # CORRECCIÃ“N CONSULTAS SQL: Uso de MAKE_INTERVAL para compatibilidad PostgreSQL
        query_mora = """
    ...
    if df_socios.empty:
        ...
        with tab3:
            ...
            if df_rep_p.empty:
                ...
                col_r3.metric(
                    "ðŸ“¥ InterÃ©s Cobrado (Mes)", f"C$ {m_int_cobrado_mes:,.2f}"
                )
                col_r4.metric(
                    "ðŸ“Š Cumplimiento de InterÃ©s", f"{pct_cumplimiento:.1f}%"
                )
...
# ==========================================
            ...
            with st.form("form_pago", clear_on_submit=True):
                ...
                if btn_pago:
                    if tipo_pago == "Completo (Cuota Mensual)":
                        ...
                        m_capital = 0.0
                        tipo_db = "InterÃ©s"
                    else:  # Abono a Capital o CancelaciÃ³n Total
    ...
    with tab2:
        ...
        SELECT pg.id as "ID", s.nombre as "Socio", pg.prestamo_id as "ID PrÃ©stamo",
               ...
               COALESCE(pg.monto_interes, 0.00) as "Abono InterÃ©s (C$)",
    ...
    if df_socios.empty:
        ...
        with motor.connect() as conn:
            df_prestamos_det = pd.read_sql(
                text("""
                    SELECT p.id as "ID PrÃ©stamo", p.monto_prestado as "Capital (C$)",
...
# ==========================================
    ...
    if gran_total_ahorrado == 0:
        ...
        # CORRECCIÃ“N EN MERGE: Se evita error al integrar columnas numÃ©ricas de meses
...
# ==========================================
    ...
    with motor.connect() as conn:
        df_bit = pd.read_sql(
            text(
                ...
                ' "Usuario", accion as "AcciÃ³n / DescripciÃ³n" FROM bitacora'
    ...
    if not df_bit.empty:
        ...
        if filtro_bit.strip() != "":
            df_bit = df_bit[
                df_bit["AcciÃ³n / DescripciÃ³n"].str.contains(
