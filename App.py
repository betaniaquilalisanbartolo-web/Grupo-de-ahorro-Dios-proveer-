...
def init_db():
    with motor.begin() as conn:
        ...
        conn.execute(
            text("ALTER TABLE pagos ADD COLUMN IF NOT EXISTS monto_capital "
                 "NUMERIC(12, 2) DEFAULT 0.00;")
        )
        conn.execute(
            text("ALTER TABLE pagos ADD COLUMN IF NOT EXISTS monto_interes "
                 "NUMERIC(12, 2) DEFAULT 0.00;")
        )
        conn.execute(
...
def to_excel(df: pd.DataFrame) -> bytes:
    ...
    try:
        with pd.ExcelWriter(salida, engine="openpyxl") as escritor:
            df.to_excel(escritor, index=False, sheet_name="Reporte")
        salida.seek(0)
...
def exportar_consolidado_excel(anio_filtro: int = None) -> bytes:
    ...
    with motor.connect() as conn:
        ...
        with pd.ExcelWriter(salida, engine="openpyxl") as escritor:
            df_s.to_excel(escritor, index=False, sheet_name="Socios")
            df_a.to_excel(escritor, index=False, sheet_name="Ahorros")
            df_p.to_excel(escritor, index=False, sheet_name="Prestamos")
            df_pg.to_excel(escritor, index=False, sheet_name="Pagos")
            df_e.to_excel(escritor, index=False, sheet_name="Egresos")
        salida.seek(0)
        return salida.getvalue()
...
if not st.session_state.autenticado:
    contrasena_entrada = st.sidebar.text_input(
    ...
    if st.sidebar.button("Iniciar sesión"):
        ...
        if hash_almacenado and verificar_contrasena(
            contrasena_entrada, hash_almacenado
        ):
        ...
        if st.button("Actualizar Clave"):
            ...
            if hash_almacenado and verificar_contrasena(
                ...
                if len(pwd_nueva.strip()) >= 4:
                    ...
                    st.success(
                        "Contraseña actualizada e encriptada correctamente."
                    )
                else:
                    st.error(
...
if opcion == "📊 Panel General":
    ...
    with col_dl2:
        anio_exp = st.selectbox(
            "Seleccionar año para filtro (Opcional):", ["Todos"] + list(range(2020, 2101)), index=0
        )
...
# ==========================================
    ...
    with tab1:
        ...
        if not df_socios.empty:
            ...
            df_socios["Acción WhatsApp"] = df_socios["Teléfono"].apply(
                crear_link_wa
            )
            st.write(
...
# ==========================================
    ...
    if df_socios.empty:
        ...
        with tab2:
            ...
            consulta_ahorros = """
                SELECT a.id as "ID", s.nombre as "Socio", a.monto as "Monto (C$)", 
                       a.fecha as "Fecha", a.nota as "Nota"
                FROM ahorros a
        ...
        with tab3:
            ...
            query_edit_a = """
                SELECT a.id, s.nombre || ' - C$' || a.monto || ' (' || a.fecha || ')' as etiqueta, 
            ...
            if df_edit_a.empty:
                ...
                with st.form("form_edit_ahorro"):
                    ...
                    if btn_update_a:
                        ...
                        registrar_bitacora(
                            f"Actualización de ahorro ID {id_a_sel}"
                        )
                    ...
                    if btn_delete_a:
                        ...
                        registrar_bitacora(
                            f"Eliminación de ahorro ID {id_a_sel}"
                        )
...
# ==========================================
    ...
    if df_socios.empty:
        ...
        with tab1:
            ...
            * **Cuota Mensual Estimada:** C$ {cuota_mensual:,.2f} / mes
            ...
            if st.button("Aprobar y Registrar Préstamo"):
                ...
                st.success(
                    f"Préstamo registrado exitosamente para {socio_prestamo}"
                )
                st.rerun()
        ...
        with tab2:
            ...
            """
            with motor.connect() as conn:
                df_prestamos_hist = pd.read_sql(text(consulta_p), conn)
        ...
        with tab3:
            ...
            st.caption(
                ...
                "comparación entre el Interés Mensual esperado (%) vs. el "
                "Interés cobrado (Cumplido)."
            )
            ...
            consulta_reporte_mensual = """
                SELECT p.id AS "ID Préstamo", s.nombre AS "Socio", p.monto_prestado AS "Capital (C$)", 
                       p.tasa_interes AS "Tasa (%)", p.plazo_meses AS "Plazo (Meses)",
            ...
            if df_rep_p.empty:
                ...
                m_int_mensual_esperado = df_rep_p[
                    "Interés Mensual Esperado (C$)"
                ].sum()
                m_int_cobrado_mes = df_rep_p[
                    "Interés Cobrado en Mes (C$)"
                ].sum()
                pct_cumplimiento = (
                ...
                col_r2.metric(
                    "📈 Interés Mensual Esperado", f"C$ {m_int_mensual_esperado:,.2f}"
                )
                col_r3.metric(
                    "📥 Interés Cobrado (Mes)", f"C$ {m_int_cobrado_mes:,.2f}"
                )
                col_r4.metric(
                    "📊 Cumplimiento de Interés", f"{pct_cumplimiento:.1f}%"
                )
                ...
                st.download_button(
                    label=(
                        "📥 Descargar Reporte de Préstamos "
                        f"({mes_rep}-{anio_rep})"
                    ),
        ...
        with tab4:
            ...
            query_edit_p = """
                SELECT p.id, s.nombre || ' - Préstamo #' || p.id || ' (C$' || p.monto_prestado || ')' as etiqueta, 
            ...
            if df_edit_p.empty:
                ...
                with st.form("form_edit_prestamo"):
                    ...
                    if btn_guardar_p:
                        with motor.begin() as conn:
                            conn.execute(
                                text("""
                                    UPDATE prestamos 
                                    SET monto_prestado = :monto, tasa_interes = :tasa, 
                                        plazo_meses = :plazo, interes_total = :int_tot, 
                                        monto_total = :monto_tot, fecha_inicio = :fecha, 
                    ...
                    if btn_eliminar_p:
                        ...
                        registrar_bitacora(
                            f"Eliminación de préstamo ID {id_p_sel}"
                        )
...
# ==========================================
    ...
    if sim_tipo == "Interés Simple (Sistemas Comunitarios)":
        ...
        if r > 0:
            cuota = (
                sim_monto * (r * (1 + r) ** sim_plazo) / (((1 + r) ** sim_plazo) - 1)
            )
        else:
        ...
        c1, c2, c3 = st.columns(3)
        c1.metric("📊 Interés Total Estimado", f"C$ {total_int:,.2f}")
        c2.metric(" Total a Pagar", f"C$ {(sim_monto + total_int):,.2f}")
...
# ==========================================
    ...
    with tab1:
        consulta_activos = """
            SELECT p.id, s.nombre || ' - Préstamo #' || p.id || ' (C$' || p.monto_prestado || ' capital)' as etiqueta, 
        ...
        """
        with motor.connect() as conn:
            df_prestamos_act = pd.read_sql(text(consulta_activos), conn)
        ...
        if df_prestamos_act.empty:
            ...
            with st.form("form_pago", clear_on_submit=True):
                ...
                with motor.connect() as conn:
                    df_cap_actual = pd.read_sql(
                        text(
                            ...
                            "COALESCE(SUM(monto_pagado), 0) as tot_pagado "
                            "FROM pagos WHERE prestamo_id = :p_id"
                        ),
                        conn,
                        params={"p_id": p_id},
                    )
                ...
                interes_mensual_est = round(
                    float(datos_p["interes_total"]) / int(datos_p["plazo_meses"]), 2
                )
                capital_mensual_est = round(
                    float(datos_p["monto_prestado"]) / int(datos_p["plazo_meses"]), 2
                )
                ...
                if btn_pago:
                    ...
                    with motor.begin() as conn:
                        ...
                        df_total_p = pd.read_sql(
                            text(
                                "SELECT COALESCE(SUM(monto_capital), 0) as cap_sum, "
                                "COALESCE(SUM(monto_pagado), 0) as tot_sum "
                                "FROM pagos WHERE prestamo_id = :p_id"
                            ),
                            conn,
                            params={"p_id": p_id},
    ...
    with tab2:
        ...
        """
        with motor.connect() as conn:
            df_pagos_hist = pd.read_sql(text(consulta_pagos), conn)
    ...
    with tab3:
        ...
        query_edit_pg = """
            SELECT pg.id, s.nombre || ' - Pago#' || pg.id || ' (C$' || pg.monto_pagado || ')' as etiqueta, 
...
# ==========================================
    ...
    with tab2:
        ...
        with motor.connect() as conn:
            df_egresos_h = pd.read_sql(text(consulta_e), conn)
...
# ==========================================
    ...
    st.caption(
        "Consulta e imprime la ficha detallada de ahorro y préstamos por socio."
    )
    with motor.connect() as conn:
        df_socios = pd.read_sql(
    ...
    if df_socios.empty:
        ...
        if not df_ahorros_det.empty or not df_prestamos_det.empty:
            ...
            with pd.ExcelWriter(output_socio, engine="openpyxl") as escritor:
                df_ahorros_det.to_excel(
                    escritor, index=False, sheet_name="Ahorros"
                )
                df_prestamos_det.to_excel(
                    escritor, index=False, sheet_name="Prestamos"
                )
                if not df_pagos_det.empty:
                    df_pagos_det.to_excel(
                        escritor, index=False, sheet_name="Abonos_Prestamos"
                    )
            output_socio.seek(0)
            st.download_button(
                label=(
                    "📥 Imprimir / Descargar Estado de Cuenta de "
                    f"{socio_sel} (Excel)"
                ),
...
# ==========================================
    ...
    st.caption(
        ...
        "según el tiempo real de permanencia de los ahorros mes a mes."
    )
    ...
    if gran_total_ahorrado == 0:
        ...
        columnas_ordenadas = (
            ["ID", "Socio"] + list(meses_nombres.values()) + [
        ...
        st.success(
            "ℹ️ **Cálculo Equitativo Aplicado:** Los rendimientos se "
            "distribuirán considerando tanto el monto ahorrado como los meses "
            "que dicho capital permaneció en la caja social (Ponderación Mes a Mes)."
        )
...
# ==========================================
    ...
    with tab1:
        ...
        st.metric(
            "📥 Pagos/Cobros Recibidos en el Mes "
            f"({mes_sel}/{anio_sel})",
            f"C$ {tot_pagos_m:,.2f}",
        )
    ...
    with tab2:
        ...
        anio_cierre = st.number_input(
            "Año a Cerrar", min_value=2020, max_value=2100, value=datetime.now().year, key="cierre_anio"
        )
        ...
        if st.button("🚀 Ejecutar Cierre y Reiniciar Año"):
            ...
            st.success(
                f"¡El año {anio_cierre} ha sido cerrado correctamente! El "
                "sistema está listo para el nuevo ciclo."
            )
            st.rerun()
...
# ==========================================
    ...
    st.caption(
        "Registro de auditoría de todas las acciones y modificaciones "
    ...
    with motor.connect() as conn:
        df_bit = pd.read_sql(
            text(
                ...
                '"Usuario", accion as "Descripción" FROM bitacora '
    ...
    if not df_bit.empty:
        ...
        if filtro_bit.strip() != "":
            df_bit = df_bit[
                df_bit["Descripción"].str.contains(
