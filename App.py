...
def sincronizar_estados_prestamos():
  """Recalcula y actualiza automáticamente los préstamos cuyos abonos cubren la totalidad del capital."""
  try:
...
def to_excel(df: pd.DataFrame) -> bytes:
  ...
  try:
    ...
    st.error("Error: La librería 'openpyxl' não está instalada.")
    return b""
...
def exportar_consolidado_excel(anio_filtro: int = None) -> bytes:
  ...
  with motor.connect() as conn:
    if anio_filtro:
      ...
      df_p = pd.read_sql(
          text(
              "SELECT * FROM prestamos WHERE EXTRACT(YEAR FROM fecha_inicio) ="
              " :a"
          ),
          conn,
          params={"a": anio_filtro},
...
if not st.session_state.autenticado:
  contrasena_input = st.sidebar.text_input(
  ...
  if st.sidebar.button("Iniciar sesión"):
    ...
    if hash_almacenado and verificar_contrasena(
        contrasena_input, hash_almacenado
    ):
      ...
      if hash_almacenado and verificar_contrasena(
        ...
        if len(pwd_nueva.strip()) >= 4:
          ...
          with motor.begin() as conn:
            conn.execute(
                text(
                    "UPDATE configuracion SET valor = :v WHERE clave ="
                    " 'admin_password'"
                ),
                {"v": nuevo_hash},
            )
...
if opcion == "📊 Panel General":
  ...
  with motor.connect() as conn:
    ...
    df_capital_pagado_hist = pd.read_sql(
        text(
            "SELECT COALESCE(SUM(monto_capital), 0) as total FROM pagos pg JOIN"
            " prestamos p ON pg.prestamo_id = p.id WHERE p.estado != 'Activo'"
        ),
        conn,
    )
    ...
    df_socios = pd.read_sql(
        text("SELECT COUNT(*) as total FROM socios WHERE estado = 'Activo'"),
        conn,
    )
    total_socios = int(df_socios["total"].iloc[0])

    # CORRECCIÓN DE DISPONIBLE EN CAJA:
    # Se calcula considerando el efectivo real en caja:
    # Ahorros iniciales/totales aportados + Cobros totales recibidos (Capital + Intereses) - Préstamos Activos netos desembolsados - Egresos
    # O bien: (Total Ahorrado + Recaudado) - (Capital Activo Prestado + Egresos). Como el capital prestado activo está en la calle,
    # el disponible es: Total Ahorrado + Total Pagos Recaudados - Total Desembolsado Activo - Egresos.
    df_desembolso_activo = pd.read_sql(
        text(
            "SELECT COALESCE(SUM(monto_prestado), 0) as total FROM prestamos"
            " WHERE estado = 'Activo'"
        ),
    ...
    total_desemb_activo = float(df_desembolso_activo["total"].iloc[0])

    # Fondo en caja exacto basado en entradas y salidas de efectivo reales:
    # Efectivo en caja = (Ahorros + Total Recaudado) - (Capital Activo en la calle + Egresos)
    # Nota: Si el préstamo se saldó, su capital ya retornó a los cobros totales, por ende ya no se resta del disponible.
    fondo_caja = (total_ahorrado + total_recaudado) - (
        total_prestado + total_egresos
    )

    consulta_mora = """
            SELECT p.id as "ID", s.nombre as "Socio", p.monto_prestado as "Monto (C$)", p.fecha_inicio as "Fecha Inicio", p.plazo_meses as "Plazo (Meses)"
            FROM prestamos p 
  ...
  with col_a1:
    if not df_mora.empty:
      st.warning(
          f"⚠️ **Atención:** Se identificaron **{len(df_mora)} préstamo(s) en"
          f" MORA** (Índice de Mora: **{ratio_mora:.1f}%**)."
      )
  ...
  with col_dl2:
    ...
    st.download_button(
        ...
        file_name=(
            f"caja_ahorro_respaldo_{datetime.now().strftime('%Y%m%d')}.xlsx"
        ),
        mime=(
...
# ==========================================
  ...
  tab1, tab2, tab3 = st.tabs(
      ["📋 Listado de Socios", "➕ Registrar Nuevo Socio", "✏️ Editar / Eliminar Socio"]
  )
  ...
  with tab1:
    ...
    with motor.connect() as conn:
      df_socios = pd.read_sql(
          text(
              'SELECT id as "ID", nombre as "Nombre", telefono as "Teléfono",'
              ' fecha_registro as "Fecha Registro", estado as "Estado" FROM'
              " socios ORDER BY id ASC"
          ),
          conn,
      )
    ...
    if not df_socios.empty:
      ...
      st.download_button(
          ...
          data=to_excel(
              df_socios.drop(columns=["WhatsApp"], errors="ignore")
          ),
      ...
      if enviado:
        if nombre.strip() == "":
          ...
          with motor.begin() as conn:
            conn.execute(
                text(
                    "INSERT INTO socios (nombre, telefono, fecha_registro)"
  ...
  with tab3:
    ...
    with motor.connect() as conn:
      df_s_edit = pd.read_sql(
          text(
              "SELECT id, nombre, telefono, fecha_registro, estado FROM socios"
              " ORDER BY id DESC"
          ),
          conn,
      )
    ...
    if df_s_edit.empty:
      ...
      with st.form("form_edit_socio"):
        ...
        e_telefono = st.text_input(
            "Teléfono / WhatsApp", value=datos_socio["telefono"] or ""
        )
        ...
        if btn_guardar_edit:
          with motor.begin() as conn:
            conn.execute(
                text(
                    "UPDATE socios SET nombre = :nombre, telefono = :telefono,"
                    " fecha_registro = :fecha, estado = :estado WHERE id = :id"
                ),
                {
                    "nombre": e_nombre,
                    "telefono": e_telefono,
          ...
          registrar_bitacora(
              f"Actualización de datos del socio ID {id_socio_sel}: {e_nombre}"
          )
        ...
        if btn_eliminar_socio:
          with motor.begin() as conn:
            conn.execute(
                text(
                    "DELETE FROM pagos WHERE prestamo_id IN (SELECT id FROM"
                    " prestamos WHERE socio_id = :id)"
                ),
                {"id": id_socio_sel},
            )
            conn.execute(
          ...
          registrar_bitacora(
              f"Eliminación de socio ID {id_socio_sel}: {datos_socio['nombre']}"
          )
          st.warning(
              f"Socio ID #{id_socio_sel} y sus registros vinculados han sido"
              " eliminados."
          )
          st.rerun()
...
# ==========================================
  ...
  if df_socios.empty:
    ...
    tab1, tab2, tab3 = st.tabs(
        ["➕ Depositar Ahorro", "📜 Historial de Ahorros", "✏️ Editar / Corregir Ahorro"]
    )
      ...
      with st.form("form_ahorro", clear_on_submit=True):
        ...
        if btn_ahorro:
          ...
          with motor.begin() as conn:
            conn.execute(
                text(
                    "INSERT INTO ahorros (socio_id, monto, fecha, nota, anio)"
                    " VALUES (:socio_id, :monto, :fecha, :nota, :anio)"
                ),
                {
                    "socio_id": socio_id,
                    "monto": monto_ahorro,
          ...
          registrar_bitacora(
              f"Depósito de ahorro C$ {monto_ahorro} registrado para socio"
              f" {socio_nom}"
          )
      ...
      if df_edit_a.empty:
        ...
        with st.form("form_edit_ahorro"):
          ...
          e_monto = st.number_input(
              "Monto (C$)", value=float(reg_a["monto"]), min_value=1.0, step=10.0
          )
          ...
          if btn_update_a:
            with motor.begin() as conn:
              conn.execute(
                  text(
                      "UPDATE ahorros SET socio_id = :socio_id, monto = :monto,"
                      " fecha = :fecha, nota = :nota, anio = :anio WHERE id ="
                      " :id"
                  ),
                  {
          ...
          if btn_delete_a:
            with motor.begin() as conn:
              conn.execute(
                  text("DELETE FROM ahorros WHERE id = :id"), {"id": id_a_sel}
              )
...
# ==========================================
  ...
  if df_socios.empty:
    ...
    with tab1:
      ...
      with col1:
        ...
        monto_solicitado = st.number_input(
            "Monto del Préstamo (C$)", min_value=10.0, step=50.0
        )
        tasa_interes = st.number_input(
            "Tasa de Interés Mensual (%)", min_value=0.0, value=5.0, step=0.5
        )
      with col2:
      ...
      if st.button("Aprobar y Registrar Préstamo"):
        ...
        registrar_bitacora(
            f"Aprobación de préstamo C$ {monto_solicitado} para socio"
            f" {socio_prestamo}"
        )
    ...
    with tab2:
      ...
      query_p = """
                SELECT p.id as "ID", s.nombre as "Socio", p.monto_prestado as "Monto Prestado (C$)", p.tasa_interes as "Tasa (%)", 
                       p.plazo_meses as "Plazo (Meses)", p.interes_total as "Interés Total (C$)", p.monto_total as "Total a Pagar (C$)", 
    ...
    with tab3:
      ...
      query_reporte_mensual = """
                SELECT p.id AS "ID Préstamo", s.nombre AS "Socio", p.monto_prestado AS "Capital (C$)", p.tasa_interes AS "Tasa (%)", 
                       p.plazo_meses AS "Plazo (Meses)", (p.monto_prestado * (p.tasa_interes / 100.0)) AS "Interés Mensual Esperado (C$)",
      ...
      with motor.connect() as conn:
        df_rep_p = pd.read_sql(
            text(query_reporte_mensual), conn, params={"mes": mes_rep, "anio": anio_rep}
        )

      if df_rep_p.empty:
        st.info(
            f"No se registraron préstamos vigentes o emitidos en el mes"
            f" {mes_rep}/{anio_rep}."
        )
      else:
    ...
    with tab4:
      ...
      if df_edit_p.empty:
        ...
        prestamo_sel_e = st.selectbox(
            "Selecciona el Préstamo:", list(dict_edit_p.keys())
        )
        ...
        with st.form("form_edit_prestamo"):
          ...
          e_estado_p = st.selectbox(
              ...
              index=["Activo", "Saldado", "Cancelado"].index(reg_p["estado"]),
          )
          ...
          if btn_guardar_p:
            with motor.begin() as conn:
              conn.execute(
                  text("""
                            UPDATE prestamos SET monto_prestado = :monto, tasa_interes = :tasa, plazo_meses = :plazo, 
          ...
          if btn_eliminar_p:
            with motor.begin() as conn:
              ...
              conn.execute(
                  text("DELETE FROM prestamos WHERE id = :id"), {"id": id_p_sel}
              )
...
# ==========================================
  ...
  with col1:
    sim_monto = st.number_input(
        "Monto a Simular (C$)", min_value=100.0, value=5000.0, step=100.0
    )
    sim_tasa = st.number_input(
        "Tasa Interés Mensual (%)", min_value=0.0, value=5.0, step=0.5
    )
  with col2:
    ...
    sim_tipo = st.selectbox(
        "Método de Interés",
        [
            "Interés Simple (Sistemas Comunitarios)",
            "Amortización Cuota Fija (Francés)",
        ],
    )
    ...
    if df_prestamos_act.empty:
      ...
      with motor.connect() as conn:
        df_cap_actual = pd.read_sql(
            text(
                "SELECT COALESCE(SUM(monto_capital), 0) as cap_pagado,"
                " COALESCE(SUM(monto_pagado), 0) as tot_pagado FROM pagos WHERE"
                " prestamo_id = :p_id"
            ),
            conn,
            params={"p_id": p_id},
      ...
      if tipo_pago == "Completo (Cuota Mensual)":
        ...
        # CORRECCIÓN EN CANCELACIÓN TOTAL ANTICIPADA:
        # Al cancelar antes de tiempo, el socio solo paga el capital pendiente real + el interés del mes corriente o acordado,
        # eliminando los intereses proyectados futuros no devengados.
        monto_sugerido = round(capital_pendiente + interes_mensual_est, 2)

      st.caption(
          f"💡 **Capital pendiente actual:** C$ {capital_pendiente:,.2f} |"
          f" **Interés calculado del periodo:** C$ {interes_mensual_est:,.2f}"
      )
      ...
      with st.form("form_pago", clear_on_submit=True):
        ...
        if btn_pago:
          if tipo_pago == "Completo (Cuota Mensual)":
            ...
            # En cancelación anticipada, todo lo que exceda el interés corriente se va directamente a saldar el capital pendiente
            m_interes = min(monto_pago, interes_mensual_est)
            m_capital = max(0.0, monto_pago - m_interes)
            # Asegurar que el capital pagado ajuste exactamente el remanente si cubre o se acerca
            if (capital_pendiente - m_capital) <= 1.0:
          ...
          with motor.begin() as conn:
            ...
            df_total_p = pd.read_sql(
                text(
                    "SELECT COALESCE(SUM(monto_capital), 0) as cap_sum FROM"
                    " pagos WHERE prestamo_id = :p_id"
                ),
                conn,
                params={"p_id": p_id},
            )
            ...
            # Condición automática de saldo de préstamo
            es_saldado = (
                cap_pagado_total >= float(datos_p["monto_prestado"])
            ) or (tipo_pago == "Cancelación Total Anticipada")
            ...
            if es_saldado:
              ...
              registrar_bitacora(
                  f"Préstamo ID {p_id} de {datos_p['socio_nombre']} saldado /"
                  " cancelado anticipadamente."
              )
              st.balloons()
              st.success("🎉 ¡El préstamo ha sido cancelado/saldado completamente!")
            else:
              registrar_bitacora(
                  f"Abono de C$ {monto_pago} (Cap: C$ {m_capital}, Int:"
                  f" C$ {m_interes}) para préstamo ID {p_id}"
              )
              st.success(
                  f"Abono registrado: C$ {m_capital:,.2f} a Capital y C$"
          ...
          st.download_button(
              ...
              file_name=(
                  f"recibo_pago_{pago_id_nuevo}_{datetime.now().strftime('%Y%m%d')}.xlsx"
              ),
              mime=(
                  "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
              ),
    ...
    with tab2:
      ...
      query_pagos = """
                SELECT pg.id as "ID", s.nombre as "Socio", pg.prestamo_id as "ID Préstamo", 
                       ...
                       COALESCE(pg.monto_interes, 0.00) as "Abono Interés (C$)", pg.tipo as "Tipo", pg.fecha as "Fecha"
                FROM pagos pg 
      ...
      if df_edit_pg.empty:
        ...
        with st.form("form_edit_pago"):
          ...
          e_int_pg = st.number_input(
              "Monto Aportado a Interés (C$)",
              value=float(reg_pg["monto_interes"] or 0.0),
              min_value=0.0,
              step=10.0,
          )
          tipos_validos = ["Capital", "Interés", "Completo", "Cancelación"]
          tipo_actual = (
              reg_pg["tipo"] if reg_pg["tipo"] in tipos_validos else "Completo"
          )
          e_tipo_pg = st.selectbox(
              "Tipo", tipos_validos, index=tipos_validos.index(tipo_actual)
          )
          ...
          if btn_upd_pg:
            with motor.begin() as conn:
              conn.execute(
                  text(
                      "UPDATE pagos SET monto_pagado = :monto, monto_capital ="
                      " :cap, monto_interes = :int, tipo = :tipo, fecha ="
                      " :fecha WHERE id = :id"
                  ),
                  {
                      "monto": e_monto_pg,
          ...
          if btn_del_pg:
            with motor.begin() as conn:
              conn.execute(
                  text("DELETE FROM pagos WHERE id = :id"), {"id": id_pg_sel}
              )
...
# ==========================================
      ...
      if btn_egreso:
        if concepto.strip() == "":
          ...
          with motor.begin() as conn:
            conn.execute(
                text(
                    "INSERT INTO egresos (concepto, monto, fecha, responsable)"
                    " VALUES (:c, :m, :f, :r)"
                ),
                {
                    "c": concepto,
  ...
  with tab2:
    query_e = (
        'SELECT id as "ID", concepto as "Concepto", monto as "Monto (C$)", fecha'
        ' as "Fecha", responsable as "Responsable" FROM egresos ORDER BY id DESC'
    )
    with motor.connect() as conn:
...
# ==========================================
  ...
  if df_socios.empty:
    ...
    st.markdown("---")
    st.markdown(f"## 🏛️ Estado de Cuenta - **{socio_sel}**")
    st.markdown(
    ...
    with motor.connect() as conn:
      df_ahorros_det = pd.read_sql(
          text(
              'SELECT fecha as "Fecha", monto as "Monto (C$)", nota as "Nota"'
              " FROM ahorros WHERE socio_id = :id ORDER BY fecha DESC"
          ),
          conn,
          params={"id": s_id},
      )
    ...
    with motor.connect() as conn:
      df_prestamos_det = pd.read_sql(
          text("""
                    SELECT p.id as "ID Préstamo", p.monto_prestado as "Capital Inicial (C$)", 
                           p.interes_total as "Interés Total (C$)", p.monto_total as "Total Estimado (C$)", 
      ...
      df_pagos_det = pd.read_sql(
          text("""
                    SELECT pg.fecha as "Fecha", pg.prestamo_id as "ID Préstamo Ref.", 
                           pg.monto_pagado as "Monto Pagado (C$)", COALESCE(pg.monto_interes, 0.00) as "Abono a Interés (C$)", 
                           COALESCE(pg.monto_capital, 0.00) as "Abono a Capital (C$)", pg.tipo as "Tipo de Pago / Detalle" 
                    FROM pagos pg 
                    JOIN prestamos p ON pg.prestamo_id = p.id 
                    WHERE p.socio_id = :id 
    ...
    if not df_ahorros_det.empty or not df_prestamos_det.empty:
      ...
      st.download_button(
          ...
          file_name=(
              f"estado_cuenta_{socio_sel.replace(' ', '_')}_{datetime.now().strftime('%Y%m%d')}.xlsx"
          ),
          mime=(
              "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
          ),
...
# ==========================================
  ...
  st.caption(
      "Reparto transparente del capital acumulado e intereses repartidos según"
      " el tiempo real de permanencia de los ahorros mes a mes."
  )

  col_l1, col_l2 = st.columns(2)
  with col_l1:
    anio_liq = st.number_input(
        "Seleccionar Año de Liquidación:",
        min_value=2020,
        max_value=2100,
        value=datetime.now().year,
        key="anio_liq_input",
  ...
  with motor.connect() as conn:
    df_tot_ahorro = pd.read_sql(
        text(
            "SELECT COALESCE(SUM(monto), 0) AS total FROM ahorros WHERE"
            " EXTRACT(YEAR FROM fecha) = :a AND EXTRACT(MONTH FROM fecha) <= :m"
        ),
        conn,
    ...
    df_tot_intereses = pd.read_sql(
        text(
            "SELECT COALESCE(SUM(COALESCE(monto_interes, 0)), 0) as total FROM"
            " pagos WHERE EXTRACT(YEAR FROM fecha) = :a AND EXTRACT(MONTH FROM"
            " fecha) <= :m"
        ),
        conn,
        params={"a": anio_liq, "m": mes_corte},
  ...
  if gran_total_ahorrado == 0:
    st.warning(
        "No hay aportaciones de ahorros registrados en este período para"
    ...
    with motor.connect() as conn:
      df_a_mes = pd.read_sql(
          text(query_ahorros_mes), conn, params={"a": anio_liq, "m": mes_corte}
      )
      df_socios_act = pd.read_sql(
          text(
              "SELECT id as socio_id, nombre as socio FROM socios WHERE estado"
              " = 'Activo' ORDER BY nombre ASC"
          ),
          conn,
      )
    ...
    df_liq_base = df_socios_act.merge(df_pivot, on="socio_id", how="left").fillna(
        0
    )
    ...
    st.success(
        "ℹ️ **Cálculo Equitativo Aplicado:** Los rendimientos se distribuirán"
        " considerando tanto el monto ahorrado como los meses que dicho capital"
        " permaneció en la caja social (Ponderación Mes a Mes)."
    )
...
# ==========================================
  ...
  with tab1:
    ...
    with col_m1:
      mes_sel = st.selectbox(
          "Seleccionar Mes", list(range(1, 13)), index=datetime.now().month - 1
      )
    with col_m2:
      anio_sel = st.number_input(
          "Seleccionar Año", min_value=2020, max_value=2100, value=datetime.now().year
      )
    ...
    st.metric(
        f" Ahorros del Mes ({mes_sel}/{anio_sel})", f"C$ {tot_ahorro_m:,.2f}"
    )
    st.metric(
  ...
  with tab2:
    ...
    st.warning(
        ...
        " historial. Los ahorros y gastos se reiniciarán. Los préstamos"
        " activos mantendrán su saldo pendiente para el siguiente ciclo."
    )
    ...
    if st.button("🚀 Ejecutar Cierre y Reiniciar Año"):
      ...
      with motor.begin() as conn:
        ...
        conn.execute(
            text(
                "DELETE FROM pagos WHERE prestamo_id IN (SELECT id FROM"
                " prestamos WHERE estado IN ('Saldado', 'Cancelado'));"
            )
        )
        conn.execute(
            text(
                "DELETE FROM prestamos WHERE estado IN ('Saldado',"
                " 'Cancelado');"
            )
        )
      registrar_bitacora(
          f"Ejecutado Cierre Anual correspondiente al año {anio_cierre}"
      )
      st.success(
          f"¡El año {anio_cierre} ha sido cerrado correctamente! Préstamos"
          " activos conservados para el siguiente ciclo."
      )
      st.rerun()
    ...
    with motor.connect() as conn:
      df_hist_cierres = pd.read_sql(
          text(
              'SELECT id as "ID", anio as "Año", total_ahorrado as "Total'
...
# ==========================================
  ...
  st.caption(
      "Registro de auditoría de todas las acciones y modificaciones realizadas"
  ...
  with motor.connect() as conn:
    df_bit = pd.read_sql(
        text(
            'SELECT id as "ID", fecha as "Fecha y Hora", usuario as "Usuario",'
            " accion as "
            ' "Descripción" FROM bitacora ORDER BY id DESC LIMIT 500'
        ),
        conn,
    )
  ...
  if not df_bit.empty:
    ...
    if filtro_bit.strip() != "":
      df_bit = df_bit[
          df_bit["Descripción"].str.contains(filtro_bit, case=False, na=False)
      ]
